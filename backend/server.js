...
const qcSystemRouter = require('./src/routes/qcSystem');
...
app.use('/api/qc', authenticateToken, qcSystemRouter);
...
// Serve QC uploads through the main backend so remote clients do not depend on
// the QC FastAPI service host/port directly.
const qcUploadsDir = path.join(config.dataDir, 'qc-uploads');
if (!fs.existsSync(qcUploadsDir)) {
  fs.mkdirSync(qcUploadsDir, { recursive: true });
}
app.use('/uploads/qc', express.static(qcUploadsDir));
