...
function requireAdminOrLabManager(req, res, next) {
  if (!req.user || !['admin', 'lab_manager'].includes(req.user.role)) {
    return res.status(403).json({ message: 'Admin or Lab Manager access required' });
  }
  next();
}
...
module.exports = {
  authenticateToken,
  requireAdmin,
  requireEngineerOrAbove,
  requireAdminOrLabManager,
};
