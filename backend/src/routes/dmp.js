...
const { authenticateToken, requireAdminOrLabManager } = require('../middleware/auth');
...
function mountDmHistoricRoutes(targetRouter, prefix, sqlite) {
  ...
  targetRouter.post(`/${prefix}/options`, authenticateToken, requireAdminOrLabManager, (req, res) => {
  ...
  targetRouter.delete(`/${prefix}/options/:id`, authenticateToken, requireAdminOrLabManager, (req, res) => {
    const { getDb } = require('../models/database');
    const db = getDb();
    try {
...
router.delete('/perf-entries', authenticateToken, requireAdminOrLabManager, async (req, res) => {
  const { getDb } = require('../models/database');
