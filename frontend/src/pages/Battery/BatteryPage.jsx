...
export default function BatteryPage() {
          ...
          // Uses refs to avoid stale-closure issues inside the WS callback.
          {
            ...
            if (ocvBad || ccvBad) {
              ...
              if (msg.record.is_retest) {
                ...
                setOutOfSpecModal({ record: rec, parts, retryCount, resumeFullTest: autoRestartAfterRetestRef.current });
              } else {
                setOutOfSpecModal({ record: rec, parts, retryCount: 0, resumeFullTest: true });
  ...
  const handleRetest = useCallback((record, resumeFullTest = false) => {
    ...
    autoRestartAfterRetestRef.current = resumeFullTest;
  ...
  useEffect(() => {
    if (!running && pendingRetestRecordRef.current) {
      ...
      pendingRetestRecordRef.current = null;
      handleRetest(record, autoRestartAfterRetestRef.current);
    }
  ...
  // no out-of-spec modal is blocking, and we are not currently starting a retest.
  useEffect(() => {
    if (!running && autoRestartAfterRetestRef.current && !pendingRetestRecordRef.current && outOfSpecModal === null && retestingBatteryId === null) {
      autoRestartAfterRetestRef.current = false;
      sendMsg({ action: 'start', payload: buildParams() });
    }
  }, [running, outOfSpecModal, retestingBatteryId, sendMsg, buildParams]);
        ...
        footer={outOfSpecModal?.type === 'dim' ? [
          ...
          <Button
            ...
            onClick={() => {
              ...
              const resumeFullTest = outOfSpecModal.resumeFullTest;
              autoRestartAfterRetestRef.current = resumeFullTest;
              setOutOfSpecModal(null);
              if (running) {
                ...
                handleRetest(record, resumeFullTest);
              }
            }}
          >
            {t('batteryRetestNow')}
          ...
          <Button
            key="save-temp"
            onClick={() => {
              const shouldResume = outOfSpecModal.resumeFullTest;
              setOutOfSpecModal(null);
              if (shouldResume) {
                // Restart the full test from the next battery
                setRetestingBatteryId(null);
                autoRestartAfterRetestRef.current = false;
                sendMsg({ action: 'start', payload: buildParams() });
              } else {
                // Manual retest -> Exit retest mode immediately
                setRetestingBatteryId(null);
                autoRestartAfterRetestRef.current = false;
              }
            }}
