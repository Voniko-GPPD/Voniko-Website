...
function DMPBridgeContent() {
  const { t } = useLang();
  const { isQC, isLab, isLabManager } = useAuth();
  ...
  const handleSelectBatch = (record) => {
    setSelection(record);
    if (record?.id) {
      setActiveTab(isQC || (isLab && !isLabManager) ? 'export' : CURVE_TAB_KEY);
    }
  };
...
export default function BatteryDMPPage() {
  ...
  const allowedTabs = new Set(['dmp', 'dm2000', 'dm3000', 'perf-report']);
  const activeMainTab = useMemo(() => {
    const section = location.pathname.split('/').filter(Boolean).pop();
    return allowedTabs.has(section) ? section : 'dmp';
  }, [location.pathname]);
  ...
  return (
    <div style={{ background: '#fff', minHeight: 'calc(100vh - 112px)' }}>
      <Tabs
        ...
        onChange={(key) => navigate(`/battery-dmp/${key}`)}
        destroyInactiveTabPane
        size="large"
