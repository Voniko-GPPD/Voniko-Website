...
if not exist "backend\node_modules" (
    ...
    if not exist "backend\node_modules\http-proxy-middleware" (
        echo  [INSTALL] Installing missing backend dependency http-proxy-middleware...
        cd backend
...
echo  [..] Checking Python for qc-system...
python --version >nul 2>&1
if errorlevel 1 (
    echo  [WARN] Python not found. qc-system will not start.
) else (
    if not exist "qc-system\venv" (
        echo  [INSTALL] Creating Python venv for qc-system...
        python -m venv "qc-system\venv"
        if errorlevel 1 (
            echo  [WARN] Failed to create qc-system venv.
        ) else (
            echo  [OK] qc-system venv created.
        )
    ) else (
        echo  [OK] qc-system venv already present.
    )
    if exist "qc-system\venv" (
        echo  [INSTALL] Installing / updating qc-system Python packages...
        call "qc-system\venv\Scripts\pip.exe" install -r "qc-system\requirements.txt" --quiet
        if errorlevel 1 (
            echo  [WARN] qc-system pip install failed.
        ) else (
            echo  [OK] qc-system Python packages ready.
        )
    )
)
...
if exist "frontend\dist" (
    ...
    call pm2 start ecosystem.config.js --only voniko-backend,voniko-count-batteries,qc-system
) else (
...
echo  ^|  QC API   : http://127.0.0.1:8002/docs              ^|
echo  ^|                                                     ^|
